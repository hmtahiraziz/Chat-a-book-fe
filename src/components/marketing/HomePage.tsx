...
import { LandingSection, SectionHeader } from "@/components/marketing/LandingSection";
  ...
  return (
    <div className="landing-page min-h-screen">
      ...
      <main className="landing-main">
        {/* Hero */}
        <LandingSection compact className="landing-section--hero">
          <div className="grid gap-12 lg:grid-cols-[1fr_1.05fr] lg:items-center lg:gap-16">
            <div>
              <p className="landing-eyebrow !mb-3 !normal-case !tracking-wide">RAG for your own PDFs</p>
          ...
          <hr className="landing-section-divider mt-14 sm:mt-16 lg:mt-20" />
        </LandingSection>
        ...
        <LandingSection variant="muted">
          <SectionHeader
            eyebrow="The problem"
            title="Reading shouldn’t mean hunting"
            description="Dense PDFs are where knowledge lives—and where friction shows up. BookChat is built for people who need accurate answers from material they already own."
          />
          <ul className="mt-12 grid gap-4 sm:grid-cols-3 sm:gap-5">
            {PAIN_POINTS.map((item) => (
              <li key={item.title} className="landing-card">
                ...
                <p className="mt-2.5 text-sm leading-relaxed text-[var(--muted)]">{item.body}</p>
              </li>
            ))}
          </ul>
        </LandingSection>
        ...
        <LandingSection id="how-it-works">
          <SectionHeader
            eyebrow="Workflow"
            title="From PDF to cited answer in three steps"
            description="Upload once, chat in context, and verify every reply against the original pages."
          />
          <ol className="mt-12 grid gap-4 sm:grid-cols-3 sm:gap-5">
            {STEPS.map((item) => (
              <li key={item.step} className="landing-card flex flex-col">
                <span className="landing-card__step">{item.step}</span>
                <h3 className="mt-4 text-[17px] font-semibold text-[var(--text)]">{item.title}</h3>
        ...
        <LandingSection id="features" variant="muted">
          <SectionHeader
            eyebrow="Product"
            title="Built for study, reference, and review"
            description="The workspace, chat, and settings you use day to day—without turning your library into a black box."
          />
        ...
        {/* CTA */}
        <LandingSection variant="inset">
          <div className="landing-cta-panel flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div>
              <p className="landing-eyebrow !mb-2">Get started</p>
      ...
      <footer className="border-t border-[var(--border)] bg-[var(--surface-raised)]">
        <div className="mx-auto flex max-w-6xl flex-col gap-4 px-5 py-10 text-[12px] text-[var(--faint)] sm:flex-row sm:items-center sm:justify-between sm:px-8">
