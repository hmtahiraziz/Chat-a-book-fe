...
export function WorkspaceLibraryView() {
  ...
  const [activeTab, setActiveTab] = useState<ActiveTab>(demo ? "library" : "ingestion");
    ...
    <div className="mx-auto max-w-7xl px-4 py-8 lg:px-6">
      ...
      <header className="mb-7">
        ...
        <h1 className="font-display mt-1 text-3xl font-semibold tracking-tight text-[var(--text)] md:text-[2rem]">
          {demo ? "Demo library" : "Book Manager"}
        </h1>
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--muted)]">
          {demo ? (
            <>
              You are exploring BookChat with the sample Harry Potter book. Uploads are disabled
              on demo accounts —{" "}
              <Link href="/register" className="font-medium text-[var(--accent)] hover:underline">
                create an account
              </Link>{" "}
              to add your own PDFs. Chat on the{" "}
              <Link href="/chat" className="font-medium text-[var(--accent)] hover:underline">
                Chat
              </Link>{" "}
              screen.
            </>
      ...
      {activeTab === "ingestion" && !demo ? (
        <IngestionPanel />
      ) : (
        <LibraryPanel onGoToIngestion={demo ? undefined : () => setActiveTab("ingestion")} />
      )}
    </div>
  );
}
